...
# 1. Carga del dataset
df = pd.read_csv(
    r"C:\Users\juanj\Desktop\CreaciondeVariables-Apriori-Y-CruceVariables\CSVs\ratings2comoML.csv"
)
...
movie_support = {
    m: len(u) / total_users
    for m, u in users_by_movie.items()
}
...
def apriori_features_ultimate(row):
    ...
    for other in rated:
        ...
        if pair in frequent_pairs:
            ...
            rating_other = df[
                (df['userId'] == user) & (df['movieId'] == other)
            ]['rating'].iloc[0]
    ...
    # Construcción del Series de features
    return pd.Series({
...
# 5. Generar dataset final de características
...
# 6. Grid de hiperparámetros para RandomForest
param_grid = {
...
# 7. Iteración añadiendo variables de a una (de 4 hasta 21 features)
all_features = [
    'sup_target',
    'cnt_rated',
    'freq_pair_count',
...
extras = [f for f in all_features if f not in base_feats]

results = []

for k in range(len(extras) + 1):
    current_feats = base_feats + extras[:k]
    X_sub = X[current_feats]
...
# 8. Crear DataFrame de resultados y guardarlo
...
df_results.to_csv("dataframe_feature_iter.csv", index=False)

print(df_results)
print(f"Máximo número de features probado: {df_results['n_features'].max()}")
